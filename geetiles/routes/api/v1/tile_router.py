...
@tile_endpoints.route('/gee/<layer>/expire-cache', strict_slashes=False, methods=['DELETE'])
@is_microservice
def expire_cache(layer):
    """Expire cache tile layer Endpoint"""
    logging.info('[tile_router - expire_cache]: Expire cache tile for layer {}'.format(layer))
    logging.debug('[tile_router - expire_cache]: Clearing Redis data')
    RedisService.expire_layer(layer)
    logging.debug('[tile_router - expire_cache]: Redis data cleared, clearing Google Storage data')
    StorageService.delete_folder(layer)
    logging.debug('[tile_router - expire_cache]: Tile cache cleared')
    return "", 200
