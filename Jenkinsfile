...
node {
  ...
  try {
    ...
    stage ('Run Tests') {
      ...
      sh('docker-compose -H :2375 -f docker-compose-test.yml run -e EE_ACCOUNT=${EE_ACCOUNT} -e EE_PRIVATE_KEY=${EE_PRIVATE_KEY} -e GOOGLE_APPLICATION_CREDENTIALS=${GOOGLE_APPLICATION_CREDENTIALS} --rm test')
